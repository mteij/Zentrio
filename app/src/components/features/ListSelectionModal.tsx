...
export function ListSelectionModal({
  ...
  const checkInLists = useCallback(async () => {
    try {
      const res = await apiFetch(
        `/api/lists/check/${encodeURIComponent(item.id)}?profileId=${profileId}&t=${Date.now()}`
      )
      if (res.ok) {
        const data = await res.json()
  ...
  const handleToggleList = async (listId: number) => {
    ...
    try {
      if (isSelected) {
        // Remove
        await apiFetch(`/api/lists/${listId}/items/${encodeURIComponent(item.id)}`, {
          method: 'DELETE',
        })
      } else {
        // Add
        ...
        {loading ? (
          <div className="flex flex-col gap-2 animate-pulse">
            {[100, 140, 80].map((w, i) => (
              ...
              >
                <div className="w-7 h-7 rounded-lg bg-white/[0.05] shrink-0" />
                <div className="h-4 rounded bg-white/[0.06] flex-1" style={{ maxWidth: w }} />
                <div className="w-8 h-4 rounded bg-white/[0.04] shrink-0" />
              </div>
            ))}
          </div>
        ) : (
          ...
          >
            {lists.length === 0 && !showNewList && (
              ...
              <div className="flex flex-col items-center justify-center py-8 px-4 text-center">
                ...
                <Button variant="primary" size="small" onClick={() => setShowNewList(true)}>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <span
                      className={`text-sm font-medium truncate transition-colors duration-200 ${isInList ? 'text-white' : 'text-zinc-300 group-hover:text-white'}`}
                    >
                      {list.name}
                    </span>
                  </div>
