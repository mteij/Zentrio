...
export function StreamingDetailsStandardView(_props: {
  model: import('./Details.model').DetailsScreenModel
}) {
  ...
  const loadStreams = useCallback(
    (season?: number, episode?: number, overrideId?: string, autoPlay: boolean = false) => {
      ...
      loadStreamsProgressive(type || '', fetchId, profileId, season, episode)
    },
    ...
    try {
      ...
      if (detailsData.meta.type === 'series' && detailsData.meta.videos) {
        const seasons = Array.from(
          new Set(detailsData.meta.videos.map((v: any) => v.season || 0))
        ).sort((a: any, b: any) => a - b) as number[]
        const filteredSeasons =
          seasons.length > 1 ? seasons.filter((s: number) => s !== 0) : seasons
        ...
        if (shouldAutoPlay) {
          ...
          toast.loading(`Finding best stream for S${targetSeason}:E${targetEpisode}...`, {
            id: 'autoplay-loading',
          })
    ...
    async (stream: Stream) => {
      ...
      const doDownload = async () => {
        ...
        try {
          ...
          toast.success(
            `Downloading: ${isSeries ? `${meta.name} S${selectedEpisode!.season}:E${selectedEpisode!.number}` : meta.name}`
          )
        } catch (e) {
  ...
  const toggleWatched = async (season?: number, episode?: number) => {
    ...
    try {
      ...
      const episodeLabel =
        season !== undefined && episode !== undefined ? `S${season}:E${episode}` : data.meta.name

      if (newWatched) {
        toast.success(
          traktSynced
            ? `Marked ${episodeLabel} as watched (synced to Trakt)`
            : `Marked ${episodeLabel} as watched`,
          {
            duration: 5000,
            action: {
              label: 'Undo',
        ...
        toast.success(
          traktSynced
            ? `Marked ${episodeLabel} as unwatched (removed from Trakt)`
            : `Marked ${episodeLabel} as unwatched`,
          { duration: 3000 }
        )
      }
    } catch (e) {
