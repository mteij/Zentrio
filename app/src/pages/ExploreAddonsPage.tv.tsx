...
function isInstalled(model: ExploreAddonsScreenModel, addon: AddonRecord) {
  return model.installedAddons.find((installed) => installed.id === addon.manifest.id || installed.manifest_url === addon.transportUrl)
}
...
export function ExploreAddonsPageTvView({ model }: { model: ExploreAddonsScreenModel }) {
  ...
  const visibleAddons = model.filteredAddons.length > 0 ? model.filteredAddons : model.recommendedAddons
  ...
  return (
    ...
    >
      ...
      <TvSection title="Recommended">
        <TvGrid zoneId="addons-recommended" columns={3} nextUp="addons-filters" nextDown="addons-all">
          {model.recommendedAddons.map((addon, index) => {
            ...
            return (
              <TvFocusItem
                ...
                onActivate={() => void (installed ? model.actions.configureAddon(addon) : model.actions.installAddon(addon))}
              >
                <div className={styles.logo} style={{ backgroundImage: `url(${sanitizeImgSrc(addon.manifest.logo || '')})` }} />
                <p className={styles.addonTitle}>{addon.manifest.name}</p>
                <p className={styles.addonMeta}>{addon.manifest.description}</p>
                <div className={styles.actionRow}>
                  <span className={styles.actionPill}>{installed ? 'Installed' : 'Press to install'}</span>
      ...
      <TvSection title="All Addons" subtitle="The list respects the selected category filter.">
        {visibleAddons.length > 0 ? (
          ...
          <TvShelf zoneId="addons-all" nextUp="addons-recommended">
            <TvFocusItem id="addons-empty" className={`${styles.addonCard} ${styles.emptyCard}`} onActivate={model.actions.clearFilters}>
              <p className={styles.addonTitle}>No addons match this filter</p>
              <p className={styles.addonMeta}>Clear the current category or search filter to browse the full collection again.</p>
            </TvFocusItem>
          </TvShelf>
        )}
      </TvSection>
