...
// Mirrors the server-side 1-year session expiry; only used as a local freshness hint.
      ...
      (set, get) => ({
        ...
        updateUser: (updates) =>
          set((state) => ({
            ...
            session: state.session
              ? {
                  ...state.session,
                  user: state.user ? { ...state.user, ...updates } : state.session.user,
                }
              : null,
          })),
        ...
        refreshSession: async (token?: string) => {
          ...
          try {
            ...
            log.debug(
              `Refreshing session... Token: ${initialToken ? `...${initialToken.slice(-6)}` : 'NONE'}`
            )
            ...
            try {
              ...
              if (rawResponse.ok) {
                ...
                sessionResponse = {
                  error: { message: `HTTP ${rawResponse.status}`, status: rawResponse.status },
                }
              }
            } catch (fetchErr) {
            ...
            if (data?.user) {
              ...
              log.debug(
                'Session refreshed, token set:',
                tokenToUse ? `...${tokenToUse.slice(-6)}` : null
              )
              return true
            } else {
              ...
              if (currentToken && currentToken !== initialToken) {
                log.debug(
                  'Token changed during refresh (concurrent login detected). Keeping new session.'
                )
                set({ isLoading: false })
                return true
              }

              log.warn('Refresh failed: server returned no user — session expired.')

              // Server is up and explicitly confirmed the session is dead (200 OK, no user).
              // Do NOT preserve the local token here; that would cause an infinite 401 retry
              // loop on ProfilesPage when the token is stale after a long absence.
              // Network/server errors are handled in the catch block below.
              log.debug('Clearing local session — server confirmed it is invalid.')
              get().reset()
              return false
            }
          } catch (error: any) {
          ...
          if (state) {
            ...
            Promise.resolve().then(async () => {
              ...
              if (
                ...
                !hasSession &&
                typeof state.refreshSession === 'function'
              ) {
