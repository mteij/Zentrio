...
import { buildCatalogParameters, findGenreId } from './browse-utils'
...
export async function getCatalog(tmdbClient: TMDBClient, type: string, language: string, page: number, id: string, genre: string, config: any) {
  ...
  const parameters = await buildCatalogParameters(tmdbClient, type, language, page, id, genre, genreList, config)
