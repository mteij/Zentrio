...
(function() {
  ...
  function createAddonManagerModal() {
    ...
    const modalHTML = '<div id="' + modalId + '" class="stremio-addon-manager-modal" style="display: none;">' +
    ...
    const saveAddons = async (authKey, addons) => {
      return fetch('/stremio/api/addonCollectionSet', {
        method: 'POST',
        // Intentionally omit Content-Type to mirror reference implementation
    ...
    const displayAddons = (addons) => {
      ...
      addonsList.innerHTML = normalized.map((addon, index) => {
        ...
        '</div>';
        const officialBadge = addon.flags?.official ? '<span class="addon-badge official">Official</span>' : '';
        const protectedBadge = addon.flags?.protected ? '<span class="addon-badge protected">Protected</span>' : '';
        const descriptionHtml = addon.manifest.description ? '<p class="addon-description">' + addon.manifest.description + '</p>' : '';
        const removeButton = !addon.flags?.protected
          ? '<button class="remove-btn" data-index="' + index + '" title="Remove addon" aria-label="Remove addon">' +
              '<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">' +
                '<path d="M9 3a1 1 0 0 0-1 1v1H5.5a1 1 0 1 0 0 2H6v11a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V7h.5a1 1 0 1 0 0-2H16V4a1 1 0 0 0-1-1H9zm1 2h4V5h-4V5zm-1 4a1 1 0 1 1 2 0v8a1 1 0 1 1-2 0V9zm6-1a1 1 0 0 0-1 1v8a1 1 0 1 0 2 0V9a1 1 0 0 0-1-1z"/>' +
              '</svg>' +
            '</button>'
          : '';
        
        return '<div class="addon-item" draggable="true" data-index="' + index + '">' +
    ...
    saveBtn.addEventListener('click', async () => {
      ...
      try {
        const res = await saveAddons(authKey, currentAddons);
